...
# Matches lines that contain exam data, e.g.:
#  1 CACA001 A INTRODUCTION TO ACCOUNTING T1 N 1 PAPER ONE 02-JUN-2026 08:30 180 ...
LINE_RE = re.compile(
    r"""
    ^\s*\d+\s+          # campus code (number at start)
    ([A-Z]{2,5}\d{3,4}[A-Z0-9]*) # subject code  ← group 1
    \s+\S+\s+           # offering group (e.g. A)
    .+?                 # subject description (non-greedy)
    (\d{2}-[A-Z]{3}-\d{4}) # exam date          ← group 2
    \s+(\d{2}:\d{2})    # start time (HH:MM)    ← group 3
    """,
    re.VERBOSE,
)
...
def format_date(raw):
    """Convert '02-JUN-2026' → 'Tuesday, 02 June 2026'."""
    try:
        day, mon, year = raw.split("-")
        month_name = MONTH_MAP.get(mon.upper(), mon)
        dt = datetime.strptime(f"{day} {month_name} {year}", "%d %B %Y")
        return dt.strftime("%A, %d %B %Y")   # e.g. Tuesday, 02 June 2026
    except Exception:
        return raw

def format_time(raw):
    """Convert '08:30' → '8:30 AM', '14:00' → '2:00 PM'."""
    try:
        dt = datetime.strptime(raw, "%H:%M")
        return dt.strftime("%-I:%M %p")   # '8:30 AM' / '2:00 PM'
    except Exception:
        return raw
...
def sort_key(exam):
    ...
    try:
        return datetime.strptime(exam["_raw_date"] + " " + exam["_raw_time"], "%d-%b-%Y %H:%M")
    except Exception:
        return datetime.max

def parse_timetable(text):
    """Return list of dicts with keys: Subject, Date, Time (plus raw fields for sorting)."""
    exams = []
    seen = set()   # deduplicate identical subject+date+time combos
    ...
    for line in text.splitlines():
        ...
        raw_date  = m.group(2)          # e.g. 02-JUN-2026
        raw_time  = m.group(3)          # e.g. 08:30
...
def find_modules(all_exams, codes):
    """Filter the full exam list to only the requested module codes, sorted by date then time."""
    ...
    # Sort by actual date+time so the PDF always shows earliest exam first
...
def build_pdf(exams, period, save_folder):
    """Create a nicely formatted personal exam timetable PDF."""
    filename  = f"My {period} Exam Timetable.pdf"
...
def main():
    ...
    while True:
        ...
        # ── Step 5: generate PDF ──
        print("\n📄  Generating PDF …")
        pdf_path = build_pdf(found, period, save_folder)
        print(f"\n🎉  Done!  Saved to:\n    {pdf_path}")
        ...
        if again not in ("yes", "y"):
            print("\n👋  All done! Your timetable(s) are in the Timetables folder.")
            print(f"    {save_folder}\n")
            break
